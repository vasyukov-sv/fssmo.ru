	...
	)
	{
		...
		else
		{
			...
			if ($strWarning == '')
			{
				...
				if ($strWarning == '')
				{
					if ($ID > 0 && $PRODUCT_ID > 0 && $TMP_ID > 0 && $bCreateRecord)
					{
						...
						if (isset($parent['ID']))
						{
							...
							$check = CIBlockElement::GetList([], ["IBLOCK_ID" => $IBLOCK_ID, "ID" => $VALUES], false, false, ["ID"]);
		...
		else
		{
			...
			if ((true == isset($_POST['Update'])) && (0 < strlen($_POST['Update'])))
			{
				...
				if (!!top.Reload_SubList<?=$PRODUCT_ID ?>)
					top.Reload_SubList<?=$PRODUCT_ID ?>();
				</script><?
				die();
			}
		}
...
function GetAdminSubElementEditLink($IBLOCK_ID, $ELEMENT_ID, $SUBELEMENT_ID, $arParams = array(), $strAdd = '', $absoluteUrl = false)
{
	$absoluteUrl = ($absoluteUrl === true);
	$url = ($absoluteUrl ? '/bitrix/admin/' : '').'olympia_subelement_edit.php?IBLOCK_ID='.(int)$IBLOCK_ID.'&type='.urlencode(CIBlock::GetArrayByID($IBLOCK_ID, 'IBLOCK_TYPE_ID'));
...
$strFormAction = GetAdminSubElementEditLink(
	$IBLOCK_ID,
	$intProductID,
	($ID>0 && !$bSubCopy ? $ID : 0),
...
if ($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	<td style="white-space: nowrap;">
		<input type="text" size="50" name="NAME" id="SUB_NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="sub_name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/bitrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
	</td>
</tr><?
	$tabControl->EndCustomField("SUB_NAME", '<input type="hidden" name="NAME" id="SUB_NAME" value="'.$str_NAME.'">');
